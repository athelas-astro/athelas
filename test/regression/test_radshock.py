...
class RadShockTest(AthelasRegressionTest):
  """Test for the radiative shock"""

  def __init__(self, methodName="test_rad_shock", executable_path=None):
    ...
    build_dir = "build_rad_shock"
    run_dir = "run_rad_shock"

    # Use absolute paths for input and gold files to ensure they can be found
    infile = os.path.join(regression_dir, "test_inputs", "rad_shock.lua")
    goldfile = os.path.join(regression_dir, "goldfiles", "rad_shock.gold")
  ...
  def test_rad_shock(self):
    """Test radiative shock"""
    self.run_code()
...
def create_test_suite(executable_path=None):
  ...
  suite.addTest(RadShockTest(executable_path=executable_path))
  return suite
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser(description="Run radiative shock test")
  parser.add_argument(
    "--executable",
    "-e",
