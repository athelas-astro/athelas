...
// ---------------------------------------------------------------------------
KOKKOS_INLINE_FUNCTION std::vector<double> get_vec(const VecStore &store,
                                                   int blk_idx, int m) {
  auto h = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace{}, store);
...
KOKKOS_INLINE_FUNCTION double max_error(const VecStore &d, int N, int m,
  ...
  for (int i = 0; i < N; ++i)
    for (int j = 0; j < m; ++j)
      err = std::max(
          err,
          std::fabs(h(i, j) - x_exact[static_cast<std::size_t>(i * m + j)]));
  return err;
}
...
build_identity_block_problem(int N, int m, BlockStore &A, BlockStore &B,
