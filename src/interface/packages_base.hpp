...
class PackageWrapper {
 public:
  ...
  [[nodiscard]] auto min_timestep(const StageData &stage_data,
    ...
    if (package_->is_active()) {
      return package_->min_timestep(stage_data, dt_info);
    }
  ...
  void fill_derived(StageData &stage_data, const TimeStepInfo &dt_info) const {
    if (package_->is_active()) {
      package_->fill_derived(stage_data, dt_info);
    }
  }
 ...
 private:
  struct PackageConcept {
    ...
    virtual void fill_derived(StageData &stage_data,
...
class PackageManager {
 public:
  ...
  [[nodiscard]] auto min_timestep(const StageData &stage_data,
    ...
    for (auto *pkg : explicit_packages_) {
      if (pkg->is_active()) {
        min_dt = std::min(min_dt, pkg->min_timestep(stage_data, dt_info));
      }
    }
    for (auto *pkg : implicit_packages_) {
      if (pkg->is_active()) {
  ...
  void fill_derived(StageData &stage_data, const TimeStepInfo &dt_info) const {
    for (const auto &pkg : all_packages_) {
      if (pkg->is_active()) {
        pkg->fill_derived(stage_data, dt_info);
      }
    }
  }
