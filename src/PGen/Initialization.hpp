...
void InitializeFields( View3D uCF, View3D uPF, View3D uCR,
                       EOS *eos, GridStructure *Grid, const UInt pOrder,
                       const std::string ProblemName )
{
  ...
  if ( ProblemName == "Sod" )
  {
    ...
    const Real P_L = 40.0 / 3.0;
    const Real P_R = 0.66666e-6;
        ...
        for ( unsigned int iNodeX = 0; iNodeX < nNodes; iNodeX++ )
        {
          ...
          if ( X1 <= 0.5 )
          {
            if ( k == 0 )
            {
              ...
              Real h = 0.0;
              eos->EnthalpyFromConserved( 1.0 / D_L, V0, uCF( 2, iX, 0 ), h );
              uCF( 0, iX, 0 ) /= W;
              uCF( 1, iX, 0 ) *= W * h;
          ...
          { // right domain
            if ( k == 0 )
            {
              ...
              Real W = LorentzFactor( V0 );
              Real h = 0.0;
              eos->EnthalpyFromConserved( 1.0 / D_R, V0, uCF( 2, iX, 0 ), h );
              uCF( 0, iX, 0 ) /= W;
              uCF( 1, iX, 0 ) *= W * h;
      ...
      for ( unsigned int k = 0; k < pOrder; k++ )
        for ( unsigned int iNodeX = 0; iNodeX < nNodes; iNodeX++ )
        {
          ...
          else
          {
            ...
            Real h = 0.0;
            eos->EnthalpyFromConserved( uCF( 0, iX, 0 ), V0, uCF( 2, iX, 0 ), h );
            uCF( 0, iX, 0 ) /= W;
            uCF( 1, iX, 0 ) *= W * h;
