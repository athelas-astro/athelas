 ...
 *          diagnostic W_h = sum_q H_q phi_q (analysis::total_gravitational_energy)
 *          and the limiter energy correction. Both must use the identical phi so
 *          the reported potential energy and the correction that cancels its
 *          limiter-induced change stay consistent.
 *
 ...
 *            - Constant background:    g = -G gval     =>  phi = +G gval r.
...
auto gravitational_potential(const GravityModel model, const double gval,
  ...
  return constants::G_GRAV * gval * r;
}
