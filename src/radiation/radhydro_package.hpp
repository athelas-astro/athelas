...
KOKKOS_INLINE_FUNCTION void fixed_point_radhydro(T R, double dt_a_ii,
  ...
  while (n <= root_finders::MAX_ITERS && !converged) {
      const auto [xkp1_1, xkp1_2, xkp1_3, xkp1_4] =
          target(scratch_n);
...
fixed_point_radhydro_nodal(T R, double dt_a_ii,
                        T scratch_n, T scratch_nm1, T scratch, Args... args) {
  ...
  return std::make_tuple(
      (1.0 - omega) * u( vars::cons::Velocity) + omega * g1,
      (1.0 - omega) * u( vars::cons::Energy)   + omega * g2,
      (1.0 - omega) * u( vars::cons::RadEnergy)+ omega * g3,
  ...
  while (n < root_finders::MAX_ITERS && !converged) {
      ...
      //const auto [s_1_nm1, s_2_nm1, s_3_nm1, s_4_nm1] = target(scratch_nm1, k); // don't repeat
