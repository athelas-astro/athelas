...
void ImplicitRadiationMomentsPackage::update_implicit(
  ...
  // KINSOL-style scaled-step stopping. After applying the line-searched
  // step, check ||lam * Δ U||_inf scaled by the per-component magnitude.
  // If the step is below this tolerance, the iterate can't move
  // meaningfully in floating point: declare "solved enough" and exit so
  // the outer time integrator can proceed instead of burning the
  // remaining Newton budget on a no-op step.
  ...
  while (iter < max_iters && (!converged || !accepted)) {
    ...
    for (int ls = 0; ls < max_ls; ++ls) {
      ...
      // TODO(astrobarker): Stagnation: further halving below resolution.

      lam *= 0.5;
    } // line search

    // Apply a line-search step that was actually checked for EOS/M1
    // admissibility. If Armijo did not accept, fall back to the last
    // admissible damped step to preserve the old "best effort" behavior
    // without applying an unchecked post-halving step.
    lam = armijo_accepted ? lam : admissible_lam;
    if (lam == 0.0) {
      accepted = true;
      break;
    }
    ...
    // KINSOL-style scaled-step stopping criterion.
    // ||lam * dU||_inf, with each component scaled by max(|U_i|, U_floor).
    // If the relative step is below step_tol, further iteration can't
    // change the floats meaningfully. Treat as converged-enough so the
    // outer integrator can proceed.
