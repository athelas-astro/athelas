...
void moving_contact_init(MeshState &mesh_state, GridStructure *grid,
  ...
  athelas::par_for(
      DEFAULT_LOOP_PATTERN, "Pgen :: MovingContact (1)", DevExecSpace(),
      ib.s, ib.e, qb.s, qb.e, KOKKOS_LAMBDA(const int i, const int q) {
        ...
        if (X1 <= 0.5) {
          ...
          uCF(i, q, vars::cons::Energy) = (P_L / gm1) * uCF(i, q, vars::cons::SpecificVolume) + 0.5 * V0 * V0;

          uPF(i, q, vars::prim::Rho) = D_L;
        } else {
          uCF(i, q, vars::cons::SpecificVolume) = 1.0 / D_R;
          uCF(i, q, vars::cons::Velocity) = V0;
          uCF(i, q, vars::cons::Energy) = (P_R / gm1) * uCF(i, q, vars::cons::SpecificVolume) + 0.5 * V0 * V0;
