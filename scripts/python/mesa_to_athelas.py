...
class MESAProfile:
    ...
    def write_athelas_inputs(
        ...
        mass_frac_data = np.column_stack([self[col][::-1] for col in mass_frac_cols])
        comps_header = Z_line + "\n" + N_line
        np.savetxt(comps_file, mass_frac_data, header=comps_header, comments="")
        print(f"Wrote composition data ({len(mass_frac_cols)} species) to {comps_file}")
...
def main() -> None:
    ...
    # Load the profile
    profile = MESAProfile(logs_dir=args.logs_dir, profile_number=args.profile_number)
